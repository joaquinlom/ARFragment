...
public class MainFragmentAR extends BaseFragment implements DialogInterface.OnDismissListener{
    static
    {
    ...
    public void onActivityCreated(Bundle savedInstanceState)
    {
        ...
        int result = MetaioCloudPlugin.startJunaio(null, getActivity().getBaseContext());
        ...
        // Optionaly add GUI
        if (mGUIView==null)
            mGUIView = (RelativeLayout)getActivity().getLayoutInflater().inflate(R.layout.arview, null);
        ...
        //mMetaioCloudPluginManager.mWebview.addJavascriptInterface(new WebAppInterface(this), "Android");
        // Used to resume the camera preview
        mMetaioCloudPluginManager.mIsInLiveMode = true;

        if (result!=MetaioCloudPlugin.SUCCESS)
            Utils.showErrorForCloudPluginResult(result, getActivity());
    }
    ...
    public void onStart()
    {
        ...
        channelID = getActivity().getIntent().getIntExtra(getActivity().getPackageName()+".CHANNELID", -1);
        if (channelID>0)
        {
            // Clear the intent extra before proceeding
            getActivity().getIntent().removeExtra(getActivity().getPackageName()+".CHANNELID");
    ...
    public void onScreenshot(Bitmap bitmap, boolean saveToGalleryWithoutDialog)
    {
        ...
        String filename = "spinar-"+DateFormat.format("yyyyMMdd-hhmmss", new Date())+".jpg";
        ...
        try
        {
            ...
            if (result)
            {
                if (!saveToGalleryWithoutDialog) // meaning share it
                {
                   ...
                   // fragment.show(getActivity().getFragmentManager(),"Screen");
               }
            }
        }
        catch (Exception e)
    ...
    public MetaioWorldPOIManagerCallback getMetaioWorldPOIManagerCallback()
    {
        if (mMetaioWorldPOIManagerCallback==null)
            mMetaioWorldPOIManagerCallback = new MetaioPOIManager(getActivity());
    ...
    public ARMetaioCloudPluginManager getARMetaioCloudPluginManagerInstance()
    {
        if (mMetaioCloudPluginManager==null)
            mMetaioCloudPluginManager = new MyCloudPlugin(getActivity());
    ...
    class MetaioPOIManager extends MetaioWorldPOIManagerCallback
    {
        ...
        protected void onSaveScreenshot(Bitmap screenshot, boolean saveToGalleryWithoutDialog)
        {
            ...
            MainFragmentAR.this.onScreenshot(screenshot, saveToGalleryWithoutDialog);
        }
    }
    ...
    class MyCloudPlugin extends ARMetaioCloudPluginManager
    {
        ...
        public MetaioWorldPOIManagerCallback getMetaioWorldPOIManagerCallback()
        {
            return MainFragmentAR.this.getMetaioWorldPOIManagerCallback();
        }
        ...
        public void onServerEvent(DataSourceEvent event)
        {
            switch (event)
            {
                case DataSourceEventNoPoisReturned:
                    MetaioCloudUtils.showToast(getActivity(), getString(R.string.MSGI_POIS_NOT_FOUND));
                    break;
                case DataSourceEventServerError:
                    MetaioCloudUtils.showToast(getActivity(), getString(R.string.MSGE_TRY_AGAIN));
                    break;
                case DataSourceEventServerNotReachable:
                case DataSourceEventCouldNotResolveServer:
                    MetaioCloudUtils.showToast(getActivity(), getString(R.string.MSGW_SERVER_UNREACHABLE));
                    break;
                default:
                    break;
            }
    ...
    {
        onActivityResult(MetaioWorldPOIManagerCallback.REQUEST_POI_CONTEXT, -1, null);
    }
